...
| 3   | Git checkpoint / rollback                                           | reversibility = safe autonomy (the Replit prod-DB lesson)             | DONE (`checkpoint`/`restore` on mock + E2B shadow git, consumed by the `revert` tool; UI revert optional) | `packages/shared/src/sandbox.ts`, providers, `agentic.ts`     |
...
| 6   | Codebase grounding (grep + repo map)                                | repo graph +32.8%                                                     | DONE (`search` + `searchWorkspace`, `repo_map` + `buildRepoMap`); PageRank ranking optional               | `agent/agentic.ts`, `tools.ts`                                |
...
Also shipped: a `delete_file` tool (remove files, not just create and edit), an edit-uniqueness guard (an exact-match edit whose search text appears more than once is refused rather than silently editing the wrong place), a `repo_map` tool (workspace overview of files and their top-level symbols), and a `revert` tool (undo a turn's changes back to the starting checkpoint). The agent tool surface is now: read_file, write_file, edit_file, delete_file, list_dir, search, repo_map, run, screenshot, ask, revert, finish.
